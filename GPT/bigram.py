...
class BigramLangaugeModel(nn.Module):
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            ...
            logits, loss = self(idx_cond) # logits = 4, 8, 65

            logits = logits[:, -1, :] # get the logits for the last context of each batch item, 4, 65
...
for iter in range(max_iters):
    ...
    if iter % eval_interval == 0 or iter == max_iters - 1:
        ...
        print(f"iter: {iter}, train loss: {losses["train"]:.5f}, val loss: {losses["eval"]:.5f}")
